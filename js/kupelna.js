...
function switchHeat(){
    $.get('kupelna.php?action=chcemTeplo', function( data ) {
    ...
    $('#heat').change(function(){
        ...
        $.get('kupelna.php?action=nastavTeplo&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function switchLight(){
    $.get('kupelna.php?action=chcemLight', function( data ) {
        var test=parseInt(data);
        if(test==1){
    ...
    $('#light').change(function(){
        ...
        $.get('kupelna.php?action=nastavLight&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function setLights() {
    ...
    $.get('kupelna.php?action=chcemJas', function( data ) {
    ...
    $('#jas').change(function(){
        var hodnota = $(this).val();
        $.get('kupelna.php?action=nastavJas&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function setTemperature(){
    $.get('kupelna.php?action=chcemTemp', function( data ){
    ...
    $('#temp').change(function(){
        var hodnota=$(this).val();
        $.get('kupelna.php?action=nastavTemp&hodnota=' + hodnota, function( data ){
            if(data==1){
                alert('Updatnute');
            }
        });
...
function parseData( data ) {
    ...
    for(var i = 0; i < 20; i++) {
        ...
        var temperature = obj[i].temp6;
        var humidity=obj[i].humi6;
    ...
    for(var j=0;j<2;j++){
        ...
        var teplota = obj[j].temp6;
        var vlhkost=obj[j].humi6;
        var test=0;
        if(j==0){
            if(move==test){
