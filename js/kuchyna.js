...
function switchHeat(){
    $.get('kuchyna.php?action=chcemTeplo', function( data ) {
    ...
    $('#heat').change(function(){
        ...
        $.get('kuchyna.php?action=nastavTeplo&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function switchLight(){
    $.get('kuchyna.php?action=chcemLight', function( data ) {
        var test=parseInt(data);
        if(test==1){
    ...
    $('#light').change(function(){
        ...
        $.get('kuchyna.php?action=nastavLight&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function setLights() {
    ...
    $.get('kuchyna.php?action=chcemJas', function( data ) {
    ...
    $('#jas').change(function(){
        var hodnota = $(this).val();
        $.get('kuchyna.php?action=nastavJas&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function setTemperature(){
    $.get('kuchyna.php?action=chcemTemp', function( data ){
    ...
    $('#temp').change(function(){
        var hodnota=$(this).val();
        $.get('kuchyna.php?action=nastavTemp&hodnota=' + hodnota, function( data ){
            if(data==1){
                alert('Updatnute');
            }
        });
...
function parseData( data ) {
    ...
    for(var i = 0; i < 20; i++) {
        ...
        var temperature = obj[i].temp0;
        var humidity=obj[i].humi0;
        var pohyb=obj[i].pirs6;
        var test=0;
        if(pohyb==test){
            pohyb="Nie";
        }   else{
        ...
        $('.kuchyna').append(riadok);

        i=i+5;
    }
    for(var j=0;j<2;j++){
        var time=obj[j].timestamp;
        var move=obj[j].pirs6;
        var teplota = obj[j].temp0;
        var vlhkost=obj[j].humi0;
        var test=0;
        if(j==0){
            if(move==test){
