...
function switchHeat(){
    $.get('chodba.php?action=chcemTeplo', function( data ) {
    ...
    $('#heat').change(function(){
        ...
        $.get('chodba.php?action=nastavTeplo&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function switchLight(){
    $.get('chodba.php?action=chcemLight', function( data ) {
        var test=parseInt(data);
        if(test==1){
    ...
    $('#light').change(function(){
        ...
        $.get('chodba.php?action=nastavLight&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function setLights() {
    ...
    $.get('chodba.php?action=chcemJas', function( data ) {
    ...
    $('#jas').change(function(){
        var hodnota = $(this).val();
        $.get('chodba.php?action=nastavJas&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function setTemperature(){
    $.get('chodba.php?action=chcemTemp', function( data ){
    ...
    $('#temp').change(function(){
        var hodnota=$(this).val();
        $.get('chodba.php?action=nastavTemp&hodnota=' + hodnota, function( data ){
            if(data==1){
                alert('Updatnute');
            }
        });
...
function parseData( data ) {
    ...
    for(var i = 0; i < 20; i++) {
        ...
        var temperature = obj[i].temp3;
        var humidity=obj[i].humi3;
        var pohyb=obj[i].pirs3;
        var test=0;
        if(pohyb==test){
            pohyb="Nie";
        }   else{
        ...
        $('.chodba').append(riadok);

        i=i+5;
    }
    for(var j=0;j<2;j++){
        var time=obj[j].timestamp;
        var move=obj[j].pirs3;
        var teplota = obj[j].temp3;
        var vlhkost=obj[j].humi3;
        var test=0;
        if(j==0){
            if(move==test){
