...
function switchHeat(){
    $.get('garaz.php?action=chcemTeplo', function( data ) {
    ...
    $('#heat').change(function(){
        ...
        $.get('garaz.php?action=nastavTeplo&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function switchLight(){
    $.get('garaz.php?action=chcemLight', function( data ) {
        var test=parseInt(data);
        if(test==1){
    ...
    $('#light').change(function(){
        ...
        $.get('garaz.php?action=nastavLight&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function setLights() {
    ...
    $.get('garaz.php?action=chcemJas', function( data ) {
    ...
    $('#jas').change(function(){
        var hodnota = $(this).val();
        $.get('garaz.php?action=nastavJas&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function setTemperature(){
    $.get('garaz.php?action=chcemTemp', function( data ){
    ...
    $('#temp').change(function(){
        var hodnota=$(this).val();
        $.get('garaz.php?action=nastavTemp&hodnota=' + hodnota, function( data ){
            if(data==1){
                alert('Updatnute');
            }
        });
...
function parseData( data ) {
    ...
    for(var i = 0; i < 20; i++) {
        ...
        var temperature = obj[i].temp4;
        var humidity=obj[i].humi4;
        var pohyb=obj[i].pirs0;
        var test=0;
        if(pohyb==test){
            pohyb="Nie";
        }   else{
    ...
    for(var j=0;j<2;j++){
        ...
        var move=obj[j].pirs0;
        var teplota = obj[j].temp4;
        var vlhkost=obj[j].humi4;
        var test=0;
        if(j==0){
            if(move==test){
