...
function switchHeat(){
    $.get('spalna.php?action=chcemTeplo', function( data ) {
    ...
    $('#heat').change(function(){
        ...
        $.get('spalna.php?action=nastavTeplo&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function switchLight(){
    $.get('spalna.php?action=chcemLight', function( data ) {
        var test=parseInt(data);
        if(test==1){
    ...
    $('#light').change(function(){
        ...
        $.get('spalna.php?action=nastavLight&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function setLights() {
    ...
    $.get('spalna.php?action=chcemJas', function( data ) {
    ...
    $('#jas').change(function(){
        var hodnota = $(this).val();
        $.get('spalna.php?action=nastavJas&hodnota=' + hodnota, function( data ) {
            if (data == 1) {
                alert('Updatnute');
            }
        });
...
function setTemperature(){
    $.get('spalna.php?action=chcemTemp', function( data ){
    ...
    $('#temp').change(function(){
        var hodnota=$(this).val();
        $.get('spalna.php?action=nastavTemp&hodnota=' + hodnota, function( data ){
            if(data==1){
                alert('Updatnute');
            }
        });
...
function parseData( data ) {
    ...
    for(var i = 0; i < 20; i++) {
        ...
        var temperature = obj[i].temp2;
        var humidity=obj[i].humi2;
        var pohyb=obj[i].pirs2;
        var test=0;
        if(pohyb==test){
            pohyb="Nie";
        }   else{
        ...
        $('.spalna').append(riadok);

        i=i+5;
    }
    for(var j=0;j<2;j++){
        var time=obj[j].timestamp;
        var move=obj[j].pirs2;
        var teplota = obj[j].temp2;
        var vlhkost=obj[j].humi2;
        var test=0;
        if(j==0){
            if(move==test){
